...
#b) f(x, y) = (x2+y2)0,25 sin2(50(x2 + y2)0,1 + 1, con x, y ∈ [−100 . . . 100],
...
def funcion_aptitud(individuo):
    ...
    y = individuo.fenotipo()[1]
    f = ((x**2 + y**2)**0.25) * ((np.sin(50*(x**2 + y**2)**0.1 + 1)**2)+1)
    return  -1*f+50

def fenotipo(individuo):
    max_val = (2**(individuo.genSize/2)) - 1 #Valor maximo representable con 32 bits

    mid = individuo.genSize // 2
    
    norm1 = individuo.genotipo[:mid].uint / max_val  #Normalizo a [0,1]
    norm2 = individuo.genotipo[mid:].uint / max_val  #Normalizo a [0,1]

    fenotipo1 = norm1 * (100 - (-100)) + (-100) #Mapeo a [-100,100]
    fenotipo2 = norm2 * (100 - (-100)) + (-100) #Mapeo a [-100,100]
    fenotipo = [fenotipo1, fenotipo2]
    return fenotipo
